...
- [x] **Melhoria: seletor de cor no dialog de editar clube (2026-09-11)** — troca do `<InputText type="color">` (linha fina) pelo `ColorPicker` do PrimeVue (`format="hex"`) com swatch colorido; computed `corSelecionada` converte `#RRGGBB` ↔ hex sem `#` (formato do componente). Specs cobrindo presença/format do ColorPicker e conversão no save em `ClubesView.spec.ts` (stubs de DataTable/Column agora propagam a linha). Lint/typecheck/246 testes verdes.

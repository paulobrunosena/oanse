...
const stubs = {
  UAvatar: { template: '<span class="avatar" />' },
...
describe('FolhaSemanalRow', () => {
  it('mostra o total apenas com a presença quando não há critérios', async () => {
    const wrapper = await mountSuspended(FolhaSemanalRow, { props: props(), global: { stubs } })

    expect(wrapper.find('.badge').text()).toContain('10 pts')
  })

  it('zera o total quando o oansista faltou', async () => {
    const wrapper = await mountSuspended(FolhaSemanalRow, {
      props: props({ presente: false }),
      global: { stubs },
    })

    expect(wrapper.find('.badge').text()).toContain('0 pts')
  })
  ...
  it('recalcula o total ao editar as seções do manual', async () => {
    ...
    expect(wrapper.find('.badge').text()).toContain('16 pts')
  })

  it('habilita o botão salvar quando ainda não há folha persistida', async () => {
    const wrapper = await mountSuspended(FolhaSemanalRow, { props: props(), global: { stubs } })
  ...
  it('desabilita o botão salvar quando o formulário está limpo em relação à folha persistida', async () => {
    const wrapper = await mountSuspended(FolhaSemanalRow, {
      props: props({

 ...
 * fn_transferir_oansista via server/api/transferencias.post.ts.
 */
export function useTransferencias() {
  ...
  async function transferir(
    ...
    const { error } = await useFetch('/api/transferencias', {
      method: 'POST',
      body: { oansista_id: oansistaId, turma_destino_id: turmaDestinoId, motivo },
    })
    if (error.value) {
      throw new Error(
        (error.value as { data?: { statusMessage?: string } })?.data?.statusMessage
        ?? 'Erro ao transferir',
      )
    }
  }
